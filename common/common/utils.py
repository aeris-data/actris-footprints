...
def short_list_repr(l, n=3):
    if not isinstance(l, (list, tuple)):
        l = list(l)
    if not isinstance(n, int) or n <= 0:
        raise ValueError(f'n must be an integer > 0; got {n}')
    if len(l) > 2 * n:
        return str(l[:n])[:-1] + ', ..., ' + str(l[-n:])[1:]
    else:
        return str(l)
...
def parallel_for_each_bag_2(action, *iterables, dask_client=None, npartitions=16):
    if len(iterables) == 0:
        raise ValueError(f'len(iterables) must be > 0')
    ...
    if dask_client is None:
        ...
        for args in iterables:
            bag = dask.bag.from_sequence(args, npartitions=npartitions)
            bags.append(bag)
        first_bag = bags[0]
        remaining_bags = bags[1:]
        results = first_bag.map(action, *remaining_bags).compute()
    return results
...
def parallel_for_each_2(action, *iterables, dask_client=None, errors='raise', log_submit=False):
    """
    A thin wrapper around dask_client.map and gather, or a sequential version if dask_client is None
    :param action: a callable
    :param iterables: a sequence of lists of arguments to pass to action
    :param dask_client: a dask client or None
    :param errors: 'raise' or 'skip'
    :param log_submit: boolean; if True and dask_client is not None, log submission of tasks to cluster on the INFO level
    :return: list of results returned by action;
    if errors='skip', the missing results of failed computations are skipped
    """
    if errors not in ['raise', 'skip']:
        raise ValueError(f"errors must be 'raise' or 'skip'; got {errors}")
    if dask_client is None:
        results = []
        for args in zip(*iterables):
            if errors == 'raise':
        ...
        if log_submit:
            logger().info('Tasks submitted to the dask cluster')
        results = dask_client.gather(futures, errors=errors)
    return results
...
def make_backup(url, suffix='', limit=None):
    ...
    if current_url.exists():
        if suffix == '':
            ...
            backup_version = int(suffix[4:]) + 1
        backup_suffix = '.bak' + (str(backup_version) if backup_version != 1 else '')
        if limit is None or backup_version <= limit:
            make_backup(url, suffix=backup_suffix, limit=limit)
